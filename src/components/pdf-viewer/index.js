...
import { Dialog, Fab, Grid, Icon, LinearProgress, Slide, Tooltip } from '@mui/material';
...
const ZoomNavigator = ({ config, setConfig }) => {
  ...
  const handleScale = (type) => {
    ...
    const newScale = type === 'max' ? parseInt(scale) + parseInt(scalePoint) : parseInt(scale) - parseInt(scalePoint);
    ...
    <Grid container direction="column" justify="space-between" alignItems="flex-end">
      <Grid item>
        <Tooltip title="Full screen" placement="left">
          <Fab disableRipple color="primary" aria-label="add" onClick={handleAutoScale}>
      ...
      <Grid item>
        <Tooltip title="Aumentar zoom" placement="left">
          <Fab disableRipple color="primary" aria-label="add" onClick={() => handleScale('max')}>
            <Icon>add</Icon>
          </Fab>
        </Tooltip>
      </Grid>
      ...
      <Grid item>
        <Tooltip title="Diminuir zoom" placement="left">
          <Fab disableRipple color="primary" aria-label="add" onClick={() => handleScale('min')}>
            <Icon>remove</Icon>
          </Fab>
        </Tooltip>
      </Grid>
...
const PagesNavigator = ({ classes, fileOrBase64, filename, config, onClose, setConfig }) => {
  const { fullLoading, numPages, page } = config;
  const { header, title, containerFilename, iconFilename, containerNavigation, containerPagination } = classes;

  const fileNameProcessed = String(filename).length > 30 ? `${String(filename).substr(0, 30)} ...` : filename;
  ...
  return (
    <Grid container direction="row" justify="space-between" alignItems="center" className={header} spacing={2}>
      ...
      {fullLoading && numPages > 10 && (
        <Grid item xs={12} md={6} className={containerPagination}>
          {`Pág. ${page * 10 + 1} até ${
            ...
            numPages <= 10 ? numPages : (page + 1) * 10 > numPages ? numPages : (page + 1) * 10
          } de ${numPages}`}
        </Grid>
      )}

      <Grid item xs={12} md={3}>
        <Grid container direction="row" alignItems="center" className={containerNavigation}>
          {numPages > 10 && (
            <div>
          ...
          <Tooltip title="Fazer o download" placement="bottom">
            <a href={fileOrBase64} download={filename} target={!isMobile ? '_blank' : '_self'} rel="noreferrer">
              <Fab disableRipple>
                <Icon>get_app</Icon>
              </Fab>
            </a>
...
const PDFViewer = ({ file, filename, onClose, open }) => {
  ...
  const classes = Style();
  //   const fileOrBase64 = file?.toLowerCase()?.endsWith('.pdf') ? file : `data:application/pdf;base64,${file}`;

  const [config, setConfig] = useState({
    fullLoading: false,
    numPages: null,
    scale: parseInt(`${isMobile ? 370 : 700}`, 10),
    pageNumber: 10,
    page: 0,
    minScale: 500,
    maxScale: parseInt(window.innerWidth, 10) - 50,
    scalePoint: 40,
  });
    ...
    <>
      <Dialog fullScreen open={open} onClose={onClose} TransitionComponent={Transition}>
        <Grid container direction="column" justify="flex-start" alignItems="stretch" className={classes.container}>
          ...
          <Grid item>
            <Grid container direction="row" justify="space-between" alignItems="flex-end" className={classes.content}>
              <Grid item xs={12}>
                <Document
                  file={file}
                  ...
                  loading={<LinearProgress color="primary" />}
                >
                  {CreatePages(config.numPages, config.scale, config.page).map((page, index) => (
